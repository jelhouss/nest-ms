...
async function bootstrap() {
  const app = await NestFactory.create(MailingModule, { bufferLogs: true });

  const configService = app.get(ConfigService);

  const MAILING_PORT = configService.getOrThrow<number>(
    "MAILING_PORT"
  ) as number;
  ...
  app.connectMicroservice({
    ...
    options: {
      host: "0.0.0.0",
      port: MAILING_TCP_PORT,
    },
  });

  await app.startAllMicroservices();
  await app.listen(MAILING_PORT);

  if (NODE_ENV === "development") {
    const logger = new Logger("Mailing");
